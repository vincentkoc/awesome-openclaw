...
- [secr-dev/openclaw-plugin](https://github.com/secr-dev/openclaw-plugin) - Native OpenClaw plugin for the secr secrets manager: brokers credentials from the secr API, enforces per-agent secret allowlists, and gates every tool call through an MCP gateway with allow/deny rules, rate limits, and atomic one-shot approval-grant consumption. Surfaces OpenClaw's native `requireApproval` UI for sensitive tools. Install with `openclaw plugins install npm:@secr/openclaw-plugin`. ![GitHub stars](https://img.shields.io/github/stars/secr-dev/openclaw-plugin?style=social)
